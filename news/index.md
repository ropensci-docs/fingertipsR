...
- Updated the description of the nearest_neighbours function, and
  removed the measure variable that was previously deprecated.
- Added checks to handle cases where no data is available for the
  specified combination of parameters in
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md).
- When no data is available and `rank = TRUE`, the function now returns
  an empty data frame with `Polarity`, `Rank`, and `AreaValuesCount`
  columns set to `NA`.
- Improved error message to inform users that no data is available and
  an empty data frame is returned.
- Updated examples.
- Updated area_types() function to use the short name in the API rather
  than the long name
...
- Provision of ability to turn off automatic proxy settings using
  internet explorer’s settings using `proxy_settings` argument
- bug fix to allow `rank = TRUE` when multiple ProfileIDs and
  IndicatorIDs passed to
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md)
- [`get_fingertips_api()`](https://docs.ropensci.org/fingertipsR/reference/get_fingertips_api.md)
  exported from package
- bug fix so
  [`indicator_update_information()`](https://docs.ropensci.org/fingertipsR/reference/indicator_update_information.md)
  works when an indicator is included without any update dates
- changed package maintainer

## fingertipsR 1.0.10 (2023-01-30)

- Bug fixed where character field has entirely missing values it
  defaulted to NA instead of “”
- Bug fix to allow indicator_update_information() to accept a long
  vector of IndicatorID

## fingertipsR 1.0.9 (2022-06-09)

- Bug fixes for indicators() function where many groups exist
- Fixed bug around class of the new `Time period range` field; now
  ensured to be a character field
- included indicator_update_information() to provide users with details
  of when the indicator was last updated (available from 7th June 2022)
...
- fixed warning message for multiple IndicatorIDs passed to
  indicator_metadata()
- fixed bug in indicator_metadata(IndicatorID = “All”) - thanks Luke
  Bradley for pointing it out
...
- fixed
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md)
  bug introduced in 1.0.2 where multiple IndicatorIDs and ProfileIDs are
  provided
...
- Improved flexibility of
  ...
  so new deprivation data can be drawn from website without being added
  to the package
- url_only argument added to
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md)
  function, allowing user to retrieve the API url(s) used to download
  their data
...
- default value for `AreaTypeID` in
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md)
  and
  [`deprivation_decile()`](https://docs.ropensci.org/fingertipsR/reference/deprivation_decile.md)
  removed (along with a helpful error message if `AreaTypeID` is
  missing)
- progress bar added when AreaTypeID = “All” in
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md)
...
- functions will provide informative message when they fail because of
...
- Added option for `AreaTypeID = "All"` in
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md)
- Added `ProfileID` argument to
...
- [`category_types()`](https://docs.ropensci.org/fingertipsR/reference/category_types.md)
  returns field called `CategoryType` which is joinable to
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md)
  output when `categorytype = TRUE`
- Added a retry function to handle occasions when the API times out when
  it shouldn’t
...
- [`nearest_neighbours()`](https://docs.ropensci.org/fingertipsR/reference/nearest_neighbours.md)
  now allows AreaTypeID 154 (CCG unchanged plus new 2018), but no longer
  allows AreaTypeID 153
...
- [`deprivation_decile()`](https://docs.ropensci.org/fingertipsR/reference/deprivation_decile.md)
  for `AreaTypeID = 7` (General Practice) now only contains 2015
  deprivation deciles. 2010 to 2012 have been removed

- bug fix around entering vectors of `IndicatorID`s and `ProfileID`s
  into
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md)
  function
...
- removed deprecated `inequalities` argument from
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md)
...
- nearest_neighbours() modified to include `measure` parameter - so user
  can get 15 CIPFA nn for upper tier local authorities
...
- fingertips_stats function added to give high level statistics of
  indicators in Fingertips
...
- API structure updated to include 99.8 and 95 confidence intervals.
  Reflected in the outputs of `fingertips_data`. **NOTE** earlier
  versions of the package will not work anymore because of the
  underlying change in the API structure
...
- automatically filter for `CategoryType = FALSE` in
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md) -
  this can be set to `TRUE` if needed

- rank of area and polarity of indicator returned from
  [`fingertips_data()`](https://docs.ropensci.org/fingertipsR/reference/fingertips_data.md)
  where `rank = TRUE` (polarity can also be found in
  [`indicator_metadata()`](https://docs.ropensci.org/fingertipsR/reference/indicator_metadata.md))

- `fingertips_redred` highlights which areas are statistically different
  to comparator *and* trending in the wrong direction

- [`category_types()`](https://docs.ropensci.org/fingertipsR/reference/category_types.md)
  lookup function to support ordering where categories exist (eg,
  deprivation decile)

- `areatypes_by_indicators()` to help users determine which indicators
  are available for each area type (and vice versa)
...
- data related to indicators for geographies where the data are already
  available on the Fingertips website
- indicator metadata
- deprivation data for geographies that are available on the Fingertips
  website
